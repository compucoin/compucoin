...
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // compucoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "compucoin Signed Message:\n";
...
{
    // compucoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    if(nHeight < 51)
    {
        nSubsidy =  160002 * COIN;
    }

    // Subsidy is cut in half every 10 years. 
    nSubsidy >>= (nHeight / 1802057);

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; 
static const int64 nTargetSpacing = 120; // compucoin: 2 minute block
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // compucoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // compucoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // compucoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("compucoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("compucoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static compucoinMiner(CWallet *pwallet)
{
    printf("compucoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("compucoin-miner");
    ...
    try { loop {
        ...
        printf("Running compucoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("compucoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&compucoinMiner, pwallet));
}
